...
if len(nhapnhanvat) > 1:
    ...
    def dichuyen():
        ...
        if dichuyen == 'w' and toadonhanvat[0] > 0:
            ...
            print(vuotqua)
    def dungthuoc():
        muondungthuoc = input('Bạn có muốn sử dụng gì trong hòm đồ của mình ko?(mỗi lần chỉ dc dùng 1 lần thuốc)(yes or no): ')
        print(nhanvat['balo'])
        if muondungthuoc == 'yes':
            thuocgi = input('bạn muốn dùng thuốc gì(sếp theo thứ tự 1,2 bạn chỉ cần nhập thứ tự của thuốc trong dãy: )')
            if thuocgi == '1':
                dungthuoc1 = input('bạn chắc chắn muốn dùng 1 lọ thuốc tăng công chứ??(yes or no)')
                if dungthuoc1 == 'yes':
                    print(nhanvat['balo']['thuốc tăng công'],'bạn đã sử dụng thành công')
                    nhanvat['atk'] = nhanvat['atk'] + 5
                else:
                    print('Bạn ko dùng thì thôi')
            if thuocgi == '2':
                dungthuoc2 = input('bạn chắc chắn muốn dùng 1 lọ thuốc tăng thủ chứ??(yes or no)')
                if dungthuoc2 == 'yes':
                    print(nhanvat['balo']['thuốc tăng thủ'],'bạn đã sử dụng thành công và bạn dc tăng thủ')
                    
                else:
                    print('Bạn ko dùng thì thôi')            
        else:
            print('bạn đã ko sử dụng dồ trong balo')     
    ...
    while man == 4:  
        ...
        if hopmau['toa do hop mau'][0] == toadonhanvat[0] and hopmau['toa do hop mau'][1] == toadonhanvat[1]:
            ...
            print('bây giờ máu của bạn là:',nhanvat['hp'])
        if loira[0] == toadonhanvat[0] and loira[1] == toadonhanvat[1]:
            if caichiakhoa >= 1:
                caichiakhoa = 0
                map()
